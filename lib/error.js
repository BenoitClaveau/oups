...
class RuntimeError extends Error {
    constructor(message = "Error", data = {}, error) {
        if (arguments.length == 2 && data instanceof Error) {
            error = data;
            data = {};
        }
        const entries = Object.entries(data);
        let args = entries.map(([k,v]) => k).concat(`return \`${message}\`;`);  //create arguments and function
